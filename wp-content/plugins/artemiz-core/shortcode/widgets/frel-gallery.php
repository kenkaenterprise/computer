...
class Frel_Gallery extends Widget_Base {
	...
	protected function render() {
		...
		if($layout == 'collage'){
			...
			$firstReplacer	 = '<img src="'.ARTEMIZ_CORE_SHORTCODE_URL.'assets/img/thumb-7-3.jpg" alt="" />';
			$secondReplacer	 = '<img src="'.ARTEMIZ_CORE_SHORTCODE_URL.'assets/img/thumb-69-44.jpg" alt="" />';
			...
			foreach ( $settings['gallery'] as $key => $image ) {
				$fullURL	= $image['url'];
				$imageID	= $image['id'];
				$img		= wp_get_attachment_image_src( $imageID, 'artemiz_fn_thumb-1400-0');
				$imgURL		= $img[0];
			...
			$thumb		   	= '<img style="'.$size.'" src="'.artemiz_CORE_SHORTCODE_URL.'assets/img/thumb-square.jpg" alt="" />';
